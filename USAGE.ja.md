...
dnschecker は指定したファイルにレコード情報をかくことで
HTML を生成したときにそれらの情報も含めることができます。

ここではレコード情報がかかれたファイルを
**レコード情報ファイル**
と呼ぶことにします。

レコード情報ファイルを指定するには `config.py` で
...
レコード情報ファイルは以下のフォーマットで記述します。

    IPアドレス|ホスト名|クラス|部屋|コメント

例として、上の例ででてきた `testzones/192.168.0.info` は

    192.168.0.1|chiya|||宇治抹茶
    192.168.0.2|chino|||カプチーノ
    192.168.0.3|syaro|||キリマンジャロ
    192.168.0.4|syaro|||キリマンジャロ
    192.168.0.4|rize|||テデザリゼ
